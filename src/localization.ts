...
export function normalizeLang(langOrLocale?: string): LanguageCode {
  if (!langOrLocale) return 'en';
...
export const DAYS_OF_WEEK_LOCALIZED: Record<LanguageCode, Record<string, string>> = {
  en: {
    Monday: 'Monday',
    Tuesday: 'Tuesday',
    Wednesday: 'Wednesday',
...
export const CATEGORIES_LOCALIZED: Record<LanguageCode, Record<string, string>> = {
  en: {
...
export const HOLIDAY_TRANSLATIONS: Record<string, { siName: string; taName: string; siDesc?: string; taDesc?: string }> = {
...
export function localizeHoliday(holiday: any, langOrLocale?: string) {
  ...
  let translation = HOLIDAY_TRANSLATIONS[key];
  if (!translation) {
    for (const [k, trans] of Object.entries(HOLIDAY_TRANSLATIONS)) {
  ...
  const localizedDayOfWeek = DAYS_OF_WEEK_LOCALIZED[lang][holiday.dayOfWeek] || holiday.dayOfWeek;
  const localizedCategory = CATEGORIES_LOCALIZED[lang][holiday.category] || holiday.category;
